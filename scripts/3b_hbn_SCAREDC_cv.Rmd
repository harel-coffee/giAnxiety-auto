---
title: "3b: Cross-validate SCARED-P models on HBN training data"
author: "Paul A. Bloom"
date: "June 21, 2019"
output: 
  html_document:
...
This markdown cross-validates SCARED-C models on the HBN training data (linear and logistic regressions). Model metrics are saved for each fold of cv, then best models are selected and fit using Bayesian inference in rstanarm on the entire HBN training set, to be later inspected and validated.
...
# Run cross-val on training set -- this will take a little while! 
...
cvScaredChildOutputs = data.frame(index = 1:length(trainSets), trainSet = c('complete', '3NN', '9NN'))
...
for (ii in 1:length(trainSets)){
  ...
  cvLogistic = cvManyLinearModels(inputData = trainSet, outcomeColumn = 'scaredBinChild', 
                                metric = crossEntropy, numFolds = 10, cvRounds = numRounds, modType = glm,
                                sumFormulas = sumFormulasScaredBinChild, indivFormulas = indivFormulasScaredBinChild, 
                                noGiFormulas = noGiFormulasScaredBinChild, interceptFormula = interceptFormulaScaredBinChild)
  ...
  cvScaredChildOutputs$cvLinear[ii] = list(cvLinear)
  cvScaredChildOutputs$cvLogistic[ii] = list(cvLogistic)
  ...
  hbnLogisticRegScaredChildCV = ggplot(cvLogistic[[2]], aes(x = model, y = median)) +
    geom_errorbar(aes(ymin = upr80, ymax = lwr80, color = ModelSet), width = 0, lwd = 1) +
    geom_errorbar(aes(ymin = upr95, ymax = lwr95, color = ModelSet), width = .1) +
    geom_point(size = 2) +
    facet_grid(~ModelSetOrdered, scales = 'free_x') +
  ...
  cvScaredChildOutputs$modelComparisonsLinear[ii] = list(list('sumOverNoGi' = sumOverNoGiLinear, 'sumOverInt' = sumOverIntLinear,
  ...
  cvScaredChildOutputs$modelComparisonsLogistic[ii] = list(list('sumOverNoGi' = sumOverNoGiLogistic, 'sumOverInt' = sumOverIntLogistic,
  ...
  # Logistic: GI Indiv. Items vs. No-GI
  ...
  # Logistic: GI Sum vs. GI Indiv. Items
  ...
  pdf(paste0('../plots/hbnLinearRegScaredChildCV_', trainSet$missingData[1], '.pdf'), height = 6, width = 8)
  ...
  pdf(paste0('../plots/hbnLogisticRegScaredChildCV_', trainSet$missingData[1], '.pdf'), height = 6, width = 8)
  gridExtra::grid.arrange(hbnLogisticRegScaredChildCV, pSumNoGiLogistic, pIndivNoGiLogistic, pSumIndivLogistic, 
                          layout_matrix = rbind(c(1,1,1),c(2,3,4)))
  dev.off()
}
...
save(cvScaredChildOutputs, file = '../output/cvScaredChildOutputs.rda', compress = TRUE)
```
...
cvLinear = cvScaredChildOutputs$cvLinear[[1]]

# Set up dataframe to hold model outputs
completeScaredChildLinearModFrame = data.frame(index = 1:4, modType = c('Sum', 'Indiv', 'NoGi', 'Int'))

# Pull corresponding formula for the best model from CV, fit using rstanarm::stan_glm()
completeScaredChildLinearModFrame$modObject[1] = list(stan_glm(data = trainSets[[1]],
                                         formula = sumFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'GI Sum Score']]],
                                         chains = nChains, cores = nCores))

completeScaredChildLinearModFrame$modObject[2] = list(stan_glm(data = trainSets[[1]], 
                                         formula = indivFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'GI Indiv. Items']]],
                                         chains = nChains, cores = nCores))

completeScaredChildLinearModFrame$modObject[3] = list(stan_glm(data = trainSets[[1]],
                                         formula = noGiFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'No GI Term']]], 
                                         chains = nChains, cores = nCores))

completeScaredChildLinearModFrame$modObject[4] =list(stan_glm(data = trainSets[[1]],
...
## 3NN
```{r, results='hide'}
cvLinear = cvScaredChildOutputs$cvLinear[[2]]
threeNNScaredChildLinearModFrame = data.frame(index = 1:4, modType = c('Sum', 'Indiv', 'NoGi', 'Int'))

threeNNScaredChildLinearModFrame$modObject[1] = list(stan_glm(data = trainSets[[2]],
                                         formula = sumFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'GI Sum Score']]],
                                         chains = nChains, cores = nCores))

threeNNScaredChildLinearModFrame$modObject[2] = list(stan_glm(data = trainSets[[2]], 
                                         formula = indivFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'GI Indiv. Items']]],
                                         chains = nChains, cores = nCores))

threeNNScaredChildLinearModFrame$modObject[3] = list(stan_glm(data = trainSets[[2]],
                                         formula = noGiFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'No GI Term']]], 
                                         chains = nChains, cores = nCores))

threeNNScaredChildLinearModFrame$modObject[4] =list(stan_glm(data = trainSets[[2]],
                                         formula = interceptFormula[[1]], 
                                         chains = nChains, cores = nCores))
```
...
## 9NN
```{r, results='hide'}
cvLinear = cvScaredChildOutputs$cvLinear[[3]]
NineNNScaredChildLinearModFrame = data.frame(index = 1:4, modType = c('Sum', 'Indiv', 'NoGi', 'Int'))

NineNNScaredChildLinearModFrame$modObject[1] = list(stan_glm(data = trainSets[[3]],
                                         formula = sumFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'GI Sum Score']]],
                                         chains = nChains, cores = nCores))

NineNNScaredChildLinearModFrame$modObject[2] = list(stan_glm(data = trainSets[[3]], 
                                         formula = indivFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'GI Indiv. Items']]],
                                         chains = nChains, cores = nCores))

NineNNScaredChildLinearModFrame$modObject[3] = list(stan_glm(data = trainSets[[3]],
                                         formula = noGiFormulas[[cvLinear[[3]]$modelNum[cvLinear[[3]]$ModelSet == 'No GI Term']]], 
                                         chains = nChains, cores = nCores))

NineNNScaredChildLinearModFrame$modObject[4] =list(stan_glm(data = trainSets[[3]],
                                         formula = interceptFormula[[1]], 
                                         chains = nChains, cores = nCores))
```
...
## COMPLETE
```{r, results='hide'}
cvLogistic = cvScaredChildOutputs$cvLogistic[[1]]
completeScaredChildLogisticModFrame = data.frame(index = 1:4, modType = c('Sum', 'Indiv', 'NoGi', 'Int'))

completeScaredChildLogisticModFrame$modObject[1] = list(stan_glm(data = trainSets[[1]],
                                         formula = sumFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'GI Sum Score']]],
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

completeScaredChildLogisticModFrame$modObject[2] = list(stan_glm(data = trainSets[[1]], 
                                         formula = indivFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'GI Indiv. Items']]],
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

completeScaredChildLogisticModFrame$modObject[3] = list(stan_glm(data = trainSets[[1]],
                                         formula = noGiFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'No GI Term']]], 
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

completeScaredChildLogisticModFrame$modObject[4] =list(stan_glm(data = trainSets[[1]],
                                         formula = interceptFormulaScaredBinChild[[1]], 
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))
```

## 3NN
```{r, results='hide'}
cvLogistic = cvScaredChildOutputs$cvLogistic[[2]]
threeNNScaredChildLogisticModFrame = data.frame(index = 1:4, modType = c('Sum', 'Indiv', 'NoGi', 'Int'))

threeNNScaredChildLogisticModFrame$modObject[1] = list(stan_glm(data = trainSets[[2]],
                                         formula = sumFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'GI Sum Score']]],
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

threeNNScaredChildLogisticModFrame$modObject[2] = list(stan_glm(data = trainSets[[2]], 
                                         formula = indivFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'GI Indiv. Items']]],
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

threeNNScaredChildLogisticModFrame$modObject[3] = list(stan_glm(data = trainSets[[2]],
                                         formula = noGiFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'No GI Term']]], 
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

threeNNScaredChildLogisticModFrame$modObject[4] =list(stan_glm(data = trainSets[[2]],
                                         formula = interceptFormulaScaredBinChild[[1]], 
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))
```
...
## 9NN
```{r, results='hide'}
cvLogistic = cvScaredChildOutputs$cvLogistic[[3]]
NineNNScaredChildLogisticModFrame = data.frame(index = 1:4, modType = c('Sum', 'Indiv', 'NoGi', 'Int'))

NineNNScaredChildLogisticModFrame$modObject[1] = list(stan_glm(data = trainSets[[3]],
                                         formula = sumFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'GI Sum Score']]],
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

NineNNScaredChildLogisticModFrame$modObject[2] = list(stan_glm(data = trainSets[[3]], 
                                         formula = indivFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'GI Indiv. Items']]],
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

NineNNScaredChildLogisticModFrame$modObject[3] = list(stan_glm(data = trainSets[[3]],
                                         formula = noGiFormulasScaredBinChild[[cvLogistic[[3]]$modelNum[cvLogistic[[3]]$ModelSet == 'No GI Term']]], 
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))

NineNNScaredChildLogisticModFrame$modObject[4] =list(stan_glm(data = trainSets[[3]],
                                         formula = interceptFormulaScaredBinChild[[1]], 
                                         chains = nChains, cores = nCores, family = binomial(link = 'logit')))
```
...
```{r}
completeScaredChildLinearModFrame = completeScaredChildLinearModFrame%>%
  group_by(modType) %>%
  mutate(., bayesR2 = median(bayes_R2(modObject[[1]])))


threeNNScaredChildLinearModFrame = threeNNScaredChildLinearModFrame %>%
  group_by(modType) %>%
  mutate(., bayesR2 = median(bayes_R2(modObject[[1]])))

NineNNScaredChildLinearModFrame = NineNNScaredChildLinearModFrame %>%
  group_by(modType) %>%
  mutate(., bayesR2 = median(bayes_R2(modObject[[1]])))

head(completeScaredChildLinearModFrame)
head(threeNNScaredChildLinearModFrame)
head(NineNNScaredChildLinearModFrame)
```

# Save all models out
```{r}
save(completeScaredChildLinearModFrame, threeNNScaredChildLinearModFrame, NineNNScaredChildLinearModFrame, 
     completeScaredChildLogisticModFrame, threeNNScaredChildLogisticModFrame, NineNNScaredChildLogisticModFrame,
     file = '../output/scaredChildModels.rda')
```
