# This script cross-validates several models for predicting SCARED-P outcomes with the HBN data and outputs a summary of R2 and F1 metrics.
...
# Set up grid for model tuning for RF
max_depths = [2,3]
max_features = [2,3]
min_samps = [10,15,20]
...
for i in range(n):
    ...
    # Get f1 scores for each set of prediction, store in output dataframe
...
# Extra identifier columns for output df
df['outcome'] = 'SCARED-P'
df['type'] = 'Classification'


# Save output!
df.to_csv('../../output/nonlinearClassificationScaredP.csv')


# Continuous SCARED-P outcome -----------------------------------------------------------------
# Clean up data
hbn = pd.read_csv('../../cleanData/fullHBN.csv')
...
for i in range(n):    
    ...
    # Get R2 scores for the test set for each regressor
...
# Save output!
df.to_csv('../../output/nonlinearRegressionScaredP.csv')
