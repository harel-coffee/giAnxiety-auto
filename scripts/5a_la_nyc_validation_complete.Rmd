---
title: "5a: Validation of GI-anxiety models in NYC/LA data  -- Complete Cases pipeline"
author: "Paul A. Bloom"
...
This markdown calculates validation performance metrics for GI-anxiety models trained on the HBN training data on the HBN holdout data using permutation and bootstrapping ONLY for the SCARED-P metric but for both linear/logistic models. Complete case pipeline.
...
sbelfk = makePreds(testData = sbelfk, modelsFrame = completeScaredParentLinearModFrame, modelType = 'linear')

# Make predictions (logistic)
sbelfkLogistic= makePreds(testData = dplyr::select(sbelfk, -contains('pred')), modelsFrame = completeScaredParentLogisticModFrame, modelType = 'logistic')
```
...
```{r}
pdf('../plots/sbelfkHBNPredsCompleteScaredBootstrap.pdf', height = 4, width = 8)
...
validationSummarySbElfkQ2 = bootQ2Summary

# Q2 p-values over null permuted distribution
validationSummarySbElfkQ2$pValNull[validationSummarySbElfkQ2$model == 'GI Sum'] = 
  (sum(nullQ2Parent  > scaredParentQ2Sum)+1)/(length(nullQ2Parent)+1)
validationSummarySbElfkQ2$pValNull[validationSummarySbElfkQ2$model == 'GI Indiv. Items'] = 
  (sum(nullQ2Parent  > scaredParentQ2Indiv)+1)/(length(nullQ2Parent)+1)
validationSummarySbElfkQ2$pValNull[validationSummarySbElfkQ2$model == 'No GI'] = 
  (sum(nullQ2Parent  > scaredParentQ2NoGI)+1)/((length(nullQ2Parent)+1)+1)
validationSummarySbElfkQ2$pValNull[validationSummarySbElfkQ2$model == 'Intercept Only'] = 
  (sum(nullQ2Parent  > scaredParentQ2Intercept)+1)/(length(nullQ2Parent)+1)

# Q2 p-values for GI models vs. No-GI
validationSummarySbElfkQ2$pValNoGI[validationSummarySbElfkQ2$model == 'GI Sum']  = 
  (sum(bootQ2Parent$sum < bootQ2Parent$noGi)+1)/(nrow(bootQ2Parent)+1)
validationSummarySbElfkQ2$pValNoGI[validationSummarySbElfkQ2$model == 'GI Indiv. Items'] = 
  (sum(bootQ2Parent$indiv < bootQ2Parent$noGi)+1)/(nrow(bootQ2Parent)+1)

validationSummarySbElfkQ2
```

AUC Validation Summary
```{r}
validationSummarySbElfkAUC = bootAucSummary

# null vector
nullAUC = permAucParent$sum

# p-values over null
validationSummarySbElfkAUC$pvalNull[validationSummarySbElfkAUC$model == 'GI Sum'] = 
  (sum(nullAUC > validationSummarySbElfkAUC$median[validationSummarySbElfkAUC$model == 'GI Sum'])+1)/(length(nullAUC)+1)
validationSummarySbElfkAUC$pvalNull[validationSummarySbElfkAUC$model == 'GI Indiv. Items'] = 
  (sum(nullAUC > validationSummarySbElfkAUC$median[validationSummarySbElfkAUC$model == 'GI Indiv. Items'])+1)/(length(nullAUC)+1)
validationSummarySbElfkAUC$pvalNull[validationSummarySbElfkAUC$model == 'No GI'] = 
 (sum(nullAUC > validationSummarySbElfkAUC$median[validationSummarySbElfkAUC$model == 'No GI'])+1)/(length(nullAUC)+1)
validationSummarySbElfkAUC$pvalNull[validationSummarySbElfkAUC$model == 'Intercept Only'] = 
  (sum(nullAUC > validationSummarySbElfkAUC$median[validationSummarySbElfkAUC$model == 'Intercept Only'])+1)/(length(nullAUC)+1)

# p-values of no-GI over GI models (no gi did better here)
validationSummarySbElfkAUC$pvalNoGi[validationSummarySbElfkAUC$model == 'GI Sum'] = 
  (sum(bootAucParent$sum > bootAucParent$noGi)+1)/(nrow(bootAucParent)+1)
validationSummarySbElfkAUC$pvalNoGi[validationSummarySbElfkAUC$model == 'GI Indiv. Items'] = 
  (sum(bootAucParent$indiv > bootAucParent$noGi)+1)/(nrow(bootAucParent)+1)

validationSummarySbElfkAUC
```

Log Loss Validation Summary

```{r}
validationSummarySbElfkLogLoss = bootLogLossSummary
...
# p-values over null
validationSummarySbElfkLogLoss$pvalNull[validationSummarySbElfkLogLoss$model == 'GI Sum'] = 
  (sum(nullLogLoss < validationSummarySbElfkLogLoss$median[validationSummarySbElfkLogLoss$model == 'GI Sum'])+1)/(length(nullLogLoss) +1)
validationSummarySbElfkLogLoss$pvalNull[validationSummarySbElfkLogLoss$model == 'GI Indiv. Items'] = 
  (sum(nullLogLoss < validationSummarySbElfkLogLoss$median[validationSummarySbElfkLogLoss$model == 'GI Indiv. Items'])+1)/(length(nullLogLoss) +1)
validationSummarySbElfkLogLoss$pvalNull[validationSummarySbElfkLogLoss$model == 'No GI'] = 
  (sum(nullLogLoss < validationSummarySbElfkLogLoss$median[validationSummarySbElfkLogLoss$model == 'No GI'])+1)/(length(nullLogLoss) +1)
validationSummarySbElfkLogLoss$pvalNull[validationSummarySbElfkLogLoss$model == 'Intercept Only'] = 
  (sum(nullLogLoss < validationSummarySbElfkLogLoss$median[validationSummarySbElfkLogLoss$model == 'Intercept Only'])+1)/(length(nullLogLoss) +1)


# p-values over no GI
validationSummarySbElfkLogLoss$pvalNoGi[validationSummarySbElfkLogLoss$model == 'GI Sum'] = 
  (sum(bootLogLossParent$sum > bootLogLossParent$noGi)+1)/(nrow(bootLogLossParent)+1)
validationSummarySbElfkLogLoss$pvalNoGi[validationSummarySbElfkLogLoss$model == 'GI Indiv. Items'] = 
  (sum(bootLogLossParent$indiv > bootLogLossParent$noGi)+1)/(nrow(bootLogLossParent)+1)


validationSummarySbElfkLogLoss

```

# Save validation summary outputs for reporting
```{r}
validationSummarySbElfkRMSE = bootRMSESummary
save(validationSummarySbElfkQ2, validationSummarySbElfkAUC, validationSummarySbElfkLogLoss, validationSummarySbElfkRMSE, file = '../output/validationSummarySbElfkCompleteCases.rda')
```
