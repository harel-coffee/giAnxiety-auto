---
title: "4c: Validation of GI-anxiety models in HBN holdout data  -- 9NN pipeline"
author: "Paul A. Bloom"
...
This markdown calculates validation performance metrics for GI-anxiety models trained on the HBN training data on the HBN holdout data using permutation and bootstrapping. Outputs are plots. This markdown uses models fit to the 9NN pipeline. 
...
hbnScaredChildTest = makePreds(testData = hbnScaredChildTestTemplate, modelsFrame = NineNNScaredChildLinearModFrame, modelType = 'linear')
hbnScaredParentTest = makePreds(testData = hbnScaredParentTestTemplate, modelsFrame = NineNNScaredParentLinearModFrame, modelType = 'linear')
hbnScaredChildBinTest = makePreds(testData = hbnScaredChildTestTemplate, modelsFrame = NineNNScaredChildLogisticModFrame, modelType = 'logistic')
hbnScaredParentBinTest = makePreds(testData = hbnScaredParentTestTemplate, modelsFrame = NineNNScaredChildLogisticModFrame, modelType = 'logistic')
hbnKsadsTest = makePreds(testData = hbnKsadsTest, modelsFrame = NineNNKsadsLogisticModFrame, modelType = 'logistic')
```
...
# Run permutations for each metric/outcome
...
validationSummary9NNParentChildBinComplete = left_join(aucParentChildBinComplete, logLossParentChildBinComplete) %>%
...
validationSummary9NNRMSE = bootRMSELong %>%
  group_by(reporter, model) %>%
...
```{r}
pdf('../plots/hbn9NNBootstrapPlotAllOutcomesWithIntercept.pdf', height = 4, width = 8)
...
validationSummary9NNQ2 = bootQ2Summary

# Put Q2 p-values into validation summary dataframe

# Child Q2 p-values
validationSummary9NNQ2$pValNull[validationSummary9NNQ2$reporter == 'Child' & validationSummary9NNQ2$model == 'GI Sum'] = 
  (sum(nullQ2Child  > scaredChildQ2Sum)+1)/(length(nullQ2Child)+1)
validationSummary9NNQ2$pValNull[validationSummary9NNQ2$reporter == 'Child' & validationSummary9NNQ2$model == 'GI Indiv. Items'] = 
  (sum(nullQ2Child  > scaredChildQ2Indiv)+1)/(length(nullQ2Child)+1)
validationSummary9NNQ2$pValNull[validationSummary9NNQ2$reporter == 'Child' & validationSummary9NNQ2$model == 'No GI'] = 
  (sum(nullQ2Child  > scaredChildQ2NoGI)+1)/(length(nullQ2Child)+1)
validationSummary9NNQ2$pValNull[validationSummary9NNQ2$reporter == 'Child' & validationSummary9NNQ2$model == 'Intercept Only'] = 
  (sum(nullQ2Child  > scaredChildQ2Intercept)+1)/(length(nullQ2Child)+1)

## Parent Q2 p-values
validationSummary9NNQ2$pValNull[validationSummary9NNQ2$reporter == 'Parent' & validationSummary9NNQ2$model == 'GI Sum'] = 
  (sum(nullQ2Parent  > scaredParentQ2Sum)+1)/(length(nullQ2Parent)+1)
validationSummary9NNQ2$pValNull[validationSummary9NNQ2$reporter == 'Parent' & validationSummary9NNQ2$model == 'GI Indiv. Items'] = 
  (sum(nullQ2Parent  > scaredParentQ2Indiv)+1)/(length(nullQ2Parent)+1)
validationSummary9NNQ2$pValNull[validationSummary9NNQ2$reporter == 'Parent' & validationSummary9NNQ2$model == 'No GI'] = 
  (sum(nullQ2Parent  > scaredParentQ2NoGI)+1)/(length(nullQ2Parent)+1)
validationSummary9NNQ2$pValNull[validationSummary9NNQ2$reporter == 'Parent' & validationSummary9NNQ2$model == 'Intercept Only'] = 
  (sum(nullQ2Parent  > scaredParentQ2Intercept)+1)/(length(nullQ2Parent)+1)

# Q2 Compared to No GI Model
validationSummary9NNQ2$pValNoGI[validationSummary9NNQ2$reporter == 'Child' & validationSummary9NNQ2$model == 'GI Sum']  = 
  (sum(bootQ2Child$sum < bootQ2Child$noGi)+1)/(nrow(bootQ2Child)+1)
validationSummary9NNQ2$pValNoGI[validationSummary9NNQ2$reporter == 'Child' & validationSummary9NNQ2$model == 'GI Indiv. Items'] = 
  (sum(bootQ2Child$indiv < bootQ2Child$noGi)+1)/(nrow(bootQ2Child)+1)
validationSummary9NNQ2$pValNoGI[validationSummary9NNQ2$reporter == 'Parent' & validationSummary9NNQ2$model == 'GI Sum']  = 
  (sum(bootQ2Parent$sum < bootQ2Parent$noGi)+1)/(nrow(bootQ2Parent)+1)
validationSummary9NNQ2$pValNoGI[validationSummary9NNQ2$reporter == 'Parent' & validationSummary9NNQ2$model == 'GI Indiv. Items'] = 
  (sum(bootQ2Parent$indiv < bootQ2Parent$noGi)+1)/(nrow(bootQ2Parent)+1)

validationSummary9NNQ2
```

AUC/Log Loss Validation Summaries with p-values (KSADS)
```{r}
validationSummary9NNAUC = bootAucSummary
nullAUC = permAucKsads$sum

# p-values over null
validationSummary9NNAUC$pvalNull[validationSummary9NNAUC$model == 'GI Sum'] = 
  (sum(nullAUC > validationSummary9NNAUC$median[validationSummary9NNAUC$model == 'GI Sum'])+1)/(length(nullAUC)+1)
validationSummary9NNAUC$pvalNull[validationSummary9NNAUC$model == 'GI Indiv. Items'] = 
  (sum(nullAUC > validationSummary9NNAUC$median[validationSummary9NNAUC$model == 'GI Indiv. Items'])+1)/(length(nullAUC)+1)
validationSummary9NNAUC$pvalNull[validationSummary9NNAUC$model == 'No GI'] = 
  (sum(nullAUC > validationSummary9NNAUC$median[validationSummary9NNAUC$model == 'No GI'])+1)/(length(nullAUC)+1)
validationSummary9NNAUC$pvalNull[validationSummary9NNAUC$model == 'Intercept Only'] = 
  (sum(nullAUC > validationSummary9NNAUC$median[validationSummary9NNAUC$model == 'Intercept Only'])+1)/(length(nullAUC)+1)

# p-values over no GI
validationSummary9NNAUC$pvalNoGi[validationSummary9NNAUC$model == 'GI Sum'] = 
  (sum(bootAucKsads$sum < bootAucKsads$noGi)+1)/(nrow(bootAucKsads)+1)
validationSummary9NNAUC$pvalNoGi[validationSummary9NNAUC$model == 'GI Indiv. Items'] = 
  (sum(bootAucKsads$indiv < bootAucKsads$noGi)+1)/(nrow(bootAucKsads)+1)

validationSummary9NNAUC
```
...
```{r}
validationSummary9NNLogLoss = bootLogLossSummary
nullLogLoss = permLogLossKsads$sum

# p-values over null
validationSummary9NNLogLoss$pvalNull[validationSummary9NNLogLoss$model == 'GI Sum'] = 
  (sum(nullLogLoss < validationSummary9NNLogLoss$median[validationSummary9NNLogLoss$model == 'GI Sum']) + 1)/(length(nullLogLoss)+1)
validationSummary9NNLogLoss$pvalNull[validationSummary9NNLogLoss$model == 'GI Indiv. Items'] = 
  (sum(nullLogLoss < validationSummary9NNLogLoss$median[validationSummary9NNLogLoss$model == 'GI Indiv. Items'])+1)/(length(nullLogLoss)+1)
validationSummary9NNLogLoss$pvalNull[validationSummary9NNLogLoss$model == 'No GI'] =
  (sum(nullLogLoss < validationSummary9NNLogLoss$median[validationSummary9NNLogLoss$model == 'No GI']) + 1)/(length(nullLogLoss)+1)
validationSummary9NNLogLoss$pvalNull[validationSummary9NNLogLoss$model == 'Intercept Only'] = 
  (sum(nullLogLoss < validationSummary9NNLogLoss$median[validationSummary9NNLogLoss$model == 'Intercept Only'])+1)/(length(nullLogLoss)+1)


# p-values over no GI
validationSummary9NNLogLoss$pvalNoGi[validationSummary9NNLogLoss$model == 'GI Sum'] =
  (sum(bootLogLossKsads$sum > bootLogLossKsads$noGi)+1)/(nrow(bootLogLossKsads)+1)
validationSummary9NNLogLoss$pvalNoGi[validationSummary9NNLogLoss$model == 'GI Indiv. Items'] = 
...
```{r}
save(validationSummary9NNQ2, validationSummary9NNAUC, validationSummary9NNLogLoss, validationSummary9NNRMSE, validationSummary9NNParentChildBinComplete, file = '../output/validationSummary9NN.rda')
```
