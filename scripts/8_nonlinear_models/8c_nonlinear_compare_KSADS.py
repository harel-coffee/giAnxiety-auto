# This script cross-validates several models for predicting KSADS outcomes with the HBN data and outputs a summary of R2 and F1 metrics.
...
hbn = hbn.drop(['Identifiers', 'scaredSumChild', 'scaredBinParent', 'scaredBinChild', 'scaredBinParent','ageCenter','cbclGISum'], 1).dropna(axis = 0)
hbn.reset_index(inplace = True, drop = True)
X = hbn.drop(['ksadsBin'], axis = 1)
...
for i in range(n): 
    ...
    # Cross-validate for hyperparam tuning
    ...
    # Save scores on test set for each model into output df
...
# Save output!
df.to_csv('../../output/nonlinearClassificationKsads.csv')
